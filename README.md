...
- PlantUML sources are preformatted by an Azure OpenAI GPT-5 call (`DOCWRITER_PLANTUML_REFORMAT_MODEL`) to fix line breaks, indentation, and labels before rendering; ensure the configured model is available in your Azure OpenAI resource. Prep also strips stray Markdown fences and fails early if a diagram body is empty or contains Mermaid to avoid silent render errors.
